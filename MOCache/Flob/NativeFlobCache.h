...
bool getData(
    ...
    while(bufferOffset < size){
        ...
        //std::cout <<"NativeFlobCache slotNo = "<<slotNo<<", slotOffset = "<<slotOffset<<std::endl;
...
void traverseNCache() {
    ...
    std::ofstream NCacheFile("/home/camie/secondo/bin/CacheRecord/NCache.txt",std::ios::trunc);
    if(!NCacheFile.is_open()){
        std::cout<<"NCacheFile open error!"<<std::endl;
    }

    int count = 0;
    //NCacheFile <<"***********NativeCacheEntry**************"<<std::endl;
    while(fir != las){
        count++;
        NCacheFile << count <<" ";
        NCacheFile << fir->flobId << " " << fir->size<<" " << fir->slotNo <<  std::endl;
        fir = fir->lruNext;
    }
    if(fir == las){
        count++;
