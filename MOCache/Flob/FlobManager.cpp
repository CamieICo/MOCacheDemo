...
bool FlobManager::destroyInstance() {
   ...
   __TRACE_ENTER__
    //将这一轮热数据统计结果写回gridsMap.txt
    std::ofstream fileNow("secondo/bin/AboutHotData/gridsMap.txt", std::ios::trunc);
    if(!fileNow.is_open()){
        std::cout << "Write gridsMap.txt error!" << std::endl;
        exit(1);
    }
    for(auto val : gridsMap){
        fileNow << val << std::endl;
    }
    fileNow.close();
   if (!instance) {
      __TRACE_LEAVE__
...
bool FlobManager::getData(
   ...
   if (id.mode < 2) {
      ...
      if (!ignoreCache) {
         if (fileId != nativeFlobs || !isTemp) {
            ...
            bool res = persistentFlobCache->getData(flob, dest, offset, size);
             persistentFlobCache->traversePCache();
             //persistentFlobCache->getHitMiss();
             persistentFlobCache->getUsedSize();
            ...
            bool res = nativeFlobCache->getData(flob, dest, offset, size);
             //nativeFlobCache->traverseNCache();
      ...
      if (actRead != size && file == nativeFlobFile) {
         __TRACE_LEAVE__
//          std::cout << "527 ok " << ", recOffset= "<<recOffset<<", actRead= "<<actRead<<std::endl;
//          id.print(std::cout);
//          std::cout<<std::endl;
         return true;
      }
      //std::cout << "execute here556:" << std::endl;
      //assert(actRead == size);
   } else if (id.mode == 2) {
...
*/
      ...
      if (actRead == size) {
         __TRACE_LEAVE__
//          std::cout << "561 actRead " << ", actRead= "<<actRead <<std::endl;
//          id.print(std::cout);
//          std::cout<<std::endl;
         return true;
      } else {
