...
#include "WinUnix.h"
#include "/home/camie/secondo/Tools/Flob/PersistentFlobCache.h"
...
// extern int travTimes;
// extern double allTime;
 ...
 string tempDir("tmp");
  ...
  if(persistent_flobcache_maxSize >0 && persistent_flobcache_slotSize >0 &&
    ...
    if(persistent_flobcache_slotSize > persistent_flobcache_maxSize ||
      ...
      persistent_flobcache_maxSize = persistent_flobcache_maxSize / 2;
      persistent_flobcache_slotSize = persistent_flobcache_slotSize / 2;
      persistent_flobcache_avgSize = persistent_flobcache_avgSize / 2;
      cout << persistent_flobcache_maxSize << " shi zhe er ma1 ," << persistent_flobcache_slotSize << "shi zhe er ma1 ," << persistent_flobcache_avgSize << endl;
  ...
  if(ok){
    ...
    // create sytem tables
    cacheRSTRel = new CacheRSTRel("SEC2CACHERST");
...
//       commandLE2 = commandLE; // XRIS original code
  ...
  if(hitCount + missCount == 0){
      ...
      hitratio = hitCount*1.0 / (hitCount + missCount);
  }
  double utila = usedSpace * 1.0 / (32*1024*1024);
  CacheRST *crst = new CacheRST( CmdNr, commandText, usedSpace, utila, page_In, page_Out, hitCount, missCount, hitratio);
  cacheRSTRel->append(crst, true);
  cout << "1781, hitratio here here here " << hitCount << " " << missCount << " " <<hitratio << endl; //看看是否是一条语句结束执行一次

    // std::ofstream hitRatio2("/home/camie/secondo/bin/CacheRecord/hitRatio2.txt",std::ios::app);
    // if(!hitRatio2.is_open()){
    //     std::cout<<"hitRatio2 open error!"<<std::endl;
    // }
    // hitRatio2 << hitCount << " " << missCount << " " << hitCount * 1.0 / (hitCount + missCount) << " " << page_In << " " << page_Out<< " " << travTimes << " " << allTime <<std::endl;
    // hitRatio2.close();
