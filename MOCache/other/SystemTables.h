...
/*
 * Camie is revising. //2022.11.12
 */

class CacheRST : public InfoTuple{
    int nr;
    std::string cmdStr;
    int usedSize;
    double utilization;
...
public:
    CacheRST(int num, const std::string& cmd, int used, double util, int pgIn, int pgOut, int ht, int ms, double hR):
    nr(num), cmdStr(cmd), usedSize(used), utilization(util), pageIn(pgIn), pageOut(pgOut), hit(ht), miss(ms), hitRatio(hR){
    ...
    virtual NList valueList() const
    {
        ...
        value.append( NList(usedSize) );
        value.append( NList(utilization) );
        value.append( NList(pageIn) );
        value.append( NList(pageOut) );
        value.append( NList(hit) );
        value.append( NList(miss) );
        value.append( NList(hitRatio) );
        return value;
    }

    virtual std::ostream& print(std::ostream& os) const
    {
        os << nr << sep << cmdStr << sep << hitRatio;
        return os;
    }
};

class CacheRSTRel : public SystemInfoRel
{
public:
    CacheRSTRel(const std::string& name) : SystemInfoRel(name)
    {}
    ...
    virtual void initSchema()
    {
        ...
        addAttribute("usedSize",  sym.INT() );
        addAttribute("utilization",    sym.REAL() );
        addAttribute("pageIn", sym.INT() );
        addAttribute("pageOut",     sym.INT() );
        addAttribute("hit",  sym.INT() );
        addAttribute("miss",   sym.INT() );
        addAttribute("hitRatio",    sym.REAL() );
    }
